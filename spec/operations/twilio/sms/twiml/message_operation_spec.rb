...
RSpec.describe Twilio::SMS::Twiml::MessageOperation, type: :operation do
  ...
  it "outputs twiml" do
    responder = double(reply: "oh, hello")
    expect(Twilio::SMS::Responder).to receive(:new).and_return(responder)
    expected = <<~EXPECTED
      <?xml version="1.0" encoding="UTF-8"?>
      <Response>
      <Message action=\"/twilio/sms/status/#{message.id + 2}\">oh, hello</Message>
      </Response>
    EXPECTED
